...
export default async function(ctx) {
  const apiUrl      = ctx.env.API_URL || 'https://t.alcy.cc/pc';
  ...
  if (family === 'accessoryInline') {
    return { type: 'widget', children: [{ type: 'text', text: '每日美图', maxLines: 1 }] };
  }

  const imgCacheKey = `meiritu_img_${family}`;
  const cooldownKey = `meiritu_cooldown_${family}`;
