/*
@Name：PingMe 自动化签到（已去除视频奖励部分，仅保留签到）
@Author：怎么肥事 https://raw.githubusercontent.com/ZenmoFeiShi/Qx/refs/heads/main/PingMe.js
@modify 奶思做了修改，理论支持 Qx,Loon,Surge,Egern,ShadowRocket,青龙；本版本移除了视频奖励循环
...
/*
  ...
  - 采用"当天追赶制"应对手机息屏导致的触发延迟：只要当前时间已经过了今天的目标时刻，
    ...
    用 LAST_RUN_KEY 记录"今天已经处理过"，标记时机是 checkIn 请求真正拿到服务器响应之后
    （不管业务结果是签到成功还是提示已签到），而不是一进条件判断就标记；
    如果是网络层面直接失败（超时、断网、HTTP 异常），不会被标记，下一次整点触发会继续重试，
    避免"这次网络问题导致没签上，结果被误判成今天已经跑过、后面再也不重试"的情况。
*/
...
async function startTasks() {
    ...
    return fetchApi('queryBalanceAndBonus').then(res => {
        ...
        // 走到这里说明 checkIn 请求已经真正到达服务器并拿到了响应（不是网络层失败），
        // 不管业务结果是"签到成功"还是"今天已经签过"之类的提示，都算今天已经处理过，标记一下、后面不再重试。
        ...
        if (isNode) {
            ...
            $.msg($.name + '🎉 任务完成', $.nodeNotifyMsg.join('\n'), '', {
                'open-url': '',
                'media-url': 'https://raw.githubusercontent.com/fmz200/wool_scripts/main/icons/apps/PingMe.png'
            });
        }
    }).catch(async err => {
        // 网络层失败（超时、连不上、HTTP 异常等）会走到这里；没有调用 markRanToday，
        // 所以下一次整点触发时 shouldRunNow() 依然会判定"今天还没签到"，会继续重试。
        $.nodeNotifyMsg.push(`❌ 网络请求失败，稍后会自动重试：${err.error || String(err)}`);
        if (isNode) {
            await sendMsg($.nodeNotifyMsg.join('\n'), "").then(r => console.log("通知发送完成"));
        } else {
            $.msg($.name + '❌ 任务失败', $.nodeNotifyMsg.join('\n') + '\n' + (err.error || String(err)), '', {
                'open-url': '',
                'media-url': 'https://raw.githubusercontent.com/fmz200/wool_scripts/main/icons/apps/PingMe.png'
            });
        }
