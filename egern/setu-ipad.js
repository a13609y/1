...
export default async function(ctx) {
  ...
  // systemMedium      ≈ 2:1  横版
  // systemLarge       ≈ 1:2  竖版
  // systemExtraLarge  ≈ 2:1  横版（iPad 专属，与 systemMedium 同方向，原脚本错误地归为竖版）
  let aspectRatio;
  if (family === 'systemMedium') {
    ...
    aspectRatio = 'gt1.6lt2.4';  // ← 修复：iPad 超大组件是横版，不是竖版
  } else {
    // systemSmall 及其他
  ...
  // refreshAfter：告知系统下次刷新时间（ISO 8601），iPhone 和 iPad 均需要
  const refreshAfter = new Date(Date.now() + cooldown).toISOString();
